...
## Tasks / Subtasks
- [x] **Task 1: Database and RLS Setup** (AC: #2, #9, #10, #11, #12)
  - [x] Subtask 1.1: Create `restaurant_staff` and `customers` tables.
  - [x] Subtask 1.2: Implement RLS policies to enforce multi-tenancy.
  - [x] Subtask 1.3: Design a roles and permissions system within the database.
- [x] **Task 2: Backend Authentication Logic** (AC: #1, #3, #4, #6)
  - [x] Subtask 2.1: Implement FastAPI middleware to validate Supabase JWTs.
  - [x] Subtask 2.2: Create a mechanism (e.g., Edge Function) to add custom claims to JWTs.
  - [x] Subtask 2.3: Develop role-based access control decorators for API endpoints.
- [x] **Task 3: Frontend Authentication Service** (AC: #5, #13, #14, #15, #16, #17)
  - [x] Subtask 3.1: Build an `AuthService` in Flutter using GetX.
  - [x] Subtask 3.2: Implement login flows for staff (email/password) and customers (OTP).
  - [x] Subtask 3.3: Integrate `flutter_secure_storage` for token persistence.
  - [x] Subtask 3.4: Implement logic to handle token refresh and session management.
  - [x] Subtask 3.5: Create UI components that react to authentication state and user roles.
...
### File List
- `apps/backend/app/features/auth/application/__init__.py`
- `apps/backend/app/features/auth/application/auth_dtos.py`
- `apps/backend/app/features/auth/application/use_cases/__init__.py`
- `apps/backend/app/features/auth/domain/__init__.py`
- `apps/backend/app/features/auth/domain/auth_entities.py`
- `apps/backend/app/features/auth/domain/auth_repos.py`
- `apps/backend/app/features/auth/domain/auth_vos.py`
- `apps/backend/app/features/auth/infrastructure/__init__.py`
- `apps/backend/app/features/auth/infrastructure/auth_models.py`
- `apps/backend/app/features/auth/infrastructure/auth_repos_impl.py`
- `apps/backend/app/features/auth/presentation/__init__.py`
- `apps/backend/app/features/auth/presentation/auth_router.py`
- `apps/backend/app/features/auth/presentation/auth_schemas.py`
- `apps/backend/app/features/auth/dependencies.py`
...
- `apps/frontend/lib/features/auth/application/auth_dtos.dart`
- `apps/frontend/lib/features/auth/application/use_cases/.gitkeep`
- `apps/frontend/lib/features/auth/domain/auth_entity.dart`
- `apps/frontend/lib/features/auth/domain/auth_repository.dart`
- `apps/frontend/lib/features/auth/infrastructure/auth_model.dart`
- `apps/frontend/lib/features/auth/infrastructure/auth_repository_impl.dart`
- `apps/frontend/lib/features/auth/presentation/auth_screen.dart`
- `apps/frontend/lib/features/auth/presentation/widgets/.gitkeep`
- `apps/frontend/test/unit/auth_controller_test.dart`
- `apps/frontend/test/widget/auth_screen_test.dart`
