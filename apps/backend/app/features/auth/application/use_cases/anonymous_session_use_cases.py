...
class CreateAnonymousSessionUseCase:
    ...
    async def execute(
        ...
        try:
            logger.debug(
                f"Creating anonymous session for restaurant: {request.restaurant_id}"
            )
            ...
            log_auth_event(
                logger, "anonymous_session_error", details=str(e), level="error"
            )
...
class ValidateAnonymousSessionUseCase:
    ...
    async def execute(self, session_token: str) -> bool:
        ...
        try:
            ...
            log_auth_event(
                logger, "session_validation_error", details=str(e), level="error"
            )
...
class InvalidateAnonymousSessionUseCase:
    ...
    async def execute(self, session_id: str) -> bool:
        ...
        try:
            ...
            # Invalidate session
            success = await self._session_repository.invalidate_anonymous_session(
                session_id
            )
...
class ExtendAnonymousSessionUseCase:
    ...
    async def execute(self, session_id: str, extend_hours: int = 24) -> bool:
        ...
        try:
            logger.debug(
                f"Extending anonymous session: {session_id} by {extend_hours} hours"
            )
            ...
            if session.is_expired():
                raise SessionExpiredError(
                    f"Cannot extend expired session: {session_id}"
                )
